...
export function BudgetVsActualChart({
  ...
  data: { name: string; limit: number; spent: number }[];
  currency?: string;
}) {
  if (data.length === 0) {
    ...
    <div className="h-72 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ left: 16 }}>
          ...
          <XAxis type="number" tickLine={false} axisLine={false} tick={{ fill: "#898781", fontSize: 12 }} tickFormatter={(v: number) => formatCurrency(v, currency).replace(/\.00$/, "")} />
